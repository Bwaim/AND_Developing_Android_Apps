...
public class VisualizerView extends View {
  ...
  public VisualizerView(Context context, AttributeSet attrs) {
    ...
    mBassCircle = new TrailedShape(BASS_MULTIPLIER) {
      @Override
      protected void drawThisShape(float shapeCenterX, float shapeCenterY, float currentSize,
  ...
  /**
   * This method is called by the {@link AudioInputReader} class to pass in the current fast Fourier
   * transform bytes. The array is then taken, divided up into segments, and each segment is
   * averaged to determine how big of a visual spike to display.
   * <p>
  ...
  public void updateFFT(byte[] bytes) {
    ...
    float midTotal = 0;
    for (int i = (int) (bytes.length * BASS_SEGMENT_SIZE); i < bytes.length * MID_SEGMENT_SIZE;
        i++) {
