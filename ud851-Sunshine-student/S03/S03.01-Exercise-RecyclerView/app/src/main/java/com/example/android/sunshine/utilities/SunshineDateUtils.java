...
public final class SunshineDateUtils {
  ...
  /**
   * To make it easy to query for the exact date, we normalize all dates that go into the database
  ...
  /**
   * Since all dates from the database are in UTC, we must convert the given date (in UTC timezone)
   * to the date in the local timezone. Ths function performs that conversion using the TimeZone
   * offset.
   *
  ...
  /**
   * Since all dates from the database are in UTC, we must convert the local date to the date in UTC
   * time. This function performs that conversion using the TimeZone offset.
   *
   ...
   * <p/>
   * The day string for forecast uses the following logic: For today: "Today, June 8" For tomorrow:
   * "Tomorrow" For the next 5 days: "Wednesday" (just the day name) For all days after that: "Mon,
   * Jun 8" (Mon, 8 Jun in UK, for example)
   *
  ...
  /**
   * Returns a date string in the format specified, which shows a date, without a year, abbreviated,
   * showing the full weekday.
   *
